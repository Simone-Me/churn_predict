...
## Slide 9 - Pourquoi ces modeles ?

Nous avons choisi plusieurs familles de modeles pour comparer des approches differentes. La Logistic Regression sert de modele simple et interpretable. Random Forest permet de capturer des relations non lineaires. XGBoost est souvent performant sur les donnees tabulaires et gere bien la ponderation avec `scale_pos_weight`. Le MLP est ajoute pour tester une approche Deep Learning, meme si ce n'est pas forcement le meilleur choix pour ce type de dataset.

L'idee n'etait pas seulement de chercher le meilleur score, mais de comparer un modele simple, des arbres, du boosting et un reseau de neurones.
...
## Slide 11 - Resultats au seuil 0.5
...
## Slide 16 - Limites et pistes d'amelioration

Le projet a quelques limites. La precision reste faible, donc certains clients alertes ne churneront pas vraiment. Le choix du seuil depend aussi du cout reel d'une action de retention, que nous n'avons pas dans le dataset. Enfin, les modeles sont entraines sur un dataset fixe, donc il faudrait surveiller leur performance dans le temps.

Comme ameliorations, on pourrait tester plus d'hyperparametres, calibrer les probabilites, ajouter une vraie matrice de cout metier, suivre le drift des donnees, et utiliser SHAP pour expliquer les predictions client par client.
